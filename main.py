...
with col3:
    ...
    slider_n_components = st.slider('Select the number of features', 1, min_values(
        dataset), max(int(min_values(dataset) / 2), 1))
...
with col1:
    option_model = st.selectbox(
        ...
        ('Logistic Regression', 'Decision Tree Classifier',
         'KNeighbors Classifier', 'GaussianNB', 'Random Forest Classifier')
    )
    ...
    if st.button("Train model"):
        ...
        if button_model_selection == "Train test split":
            X_train, X_test, y_train, y_test = train_test_split(
                X_pca, y, train_size=slider_train_test_split, random_state=input_random_state)
            ...
            for train_index, test_index in kf.split(X_pca):
                ...
                ps_list.append(precision_score(
                    y_test, y_pred, average='weighted'))
                rs_list.append(recall_score(
                    y_test, y_pred, average='weighted'))
            ...
            with col3:
                st.markdown("""### Data""")
                tabs = st.tabs(
                    [f"Fold {i+1}" for i in range(0, int(split_value))])
                for i in range(0, int(split_value)):
                    with tabs[i]:
                        st.bar_chart(pd.Series([ps_list[i], rs_list[i], fs_list[i]], index=[
                                     'Precision score', 'Recall score', 'F1 score']), use_container_width=True)
                        st.markdown(
                            f"""
